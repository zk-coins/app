...
describe('isReMintRejection', () => {
  it('matches the admit-time HTTP rejection wrapped by postJson', () => {
    ...
    const err = new Error(
      'POST /api/jobs/mint 422: {"error":"Re-mint into an existing asset account is not supported"}',
    );
  ...
  it('does not match unrelated mint failures', () => {
    ...
    expect(
      isReMintRejection(new Error('POST /api/jobs/mint 422: {"error":"invalid signature"}')),
    ).toBe(false);
