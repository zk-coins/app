 ...
 * succeed. The default export provides it; the real page picks the
 * single-asset or multi-asset surface from the runtime node capability.
 */
...
function SendPageInner() {
  const { MULTI_ASSET: multiAsset } = useFeatures();
...
function SendPageSingleAsset() {
  ...
  useAccountRedirect(account, router);
  ...
  const handleConfirm = useCallback(() => {
    ...
    setError(null);
    setConfirming(true);
  }, [account, balance, recipient, amount, t]);
        ...
        {confirming ? (
          ...
          >
            <p className="text-[13px] text-ink">
              {t('confirmTo', {
                amount: formatBtcCompact(Math.round(parseFloat(amount) * SATS_PER_BTC)),
                asset: 'BTC',
              })}
            </p>
